const pokémon = Object.freeze([
    ...
    const divThree = pokémon.filter( p => p.id % 3 === 0 );
    ...
    const idOver = pokémon.filter( p => p.id > 99 ).map( p => p.name );
    console.log(idOver);
    
    const poisonPoke = pokémon.filter ( p => p.types.length == 1 && p.types.includes('poison') ).map( p => p.name );
    console.log(poisonPoke);
    
    const secondFlying = pokémon.filter ( p => p.types[1] == 'flying').map( p => p.types[0]);
    console.log(secondFlying);
    
    const normalCount = pokémon.filter ( p => p.types.includes('normal')).map((currElement) => console.log(currElement));
